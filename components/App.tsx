...
const DashboardLayout: React.FC<{ 
    ...
    return (
        <div className="flex flex-col min-h-screen bg-grid-pattern text-slate-200 font-sans selection:bg-cyan-500/30 relative overflow-y-auto">
            <IngestionManager /> 
            ...
            <div className={`flex-1 flex flex-col lg:flex-row relative min-h-0 ${subscriptionStatus === 'BLOCKED' ? 'blur-sm pointer-events-none select-none' : ''}`}>
                ...
                <div className="hidden lg:block h-full shrink-0 border-r border-white/5 bg-slate-900/40 overflow-y-auto">
                    <Sidebar isTrialMode={isTrialMode} />
                </div>

                {/* CONTEÚDO DINÂMICO */}
                <div className="flex-1 relative flex flex-col bg-slate-950 overflow-y-auto lg:overflow-y-auto">
                   
                   {/* Mobile Views Switcher */}
                   <div className="flex-1 pb-24 lg:pb-0 overflow-y-auto">
                      {/* Aba Scanner (Default Desktop) */}
                      {(activeTab === 'scanner' || window.innerWidth > 1024) && (
                         <div className={`${activeTab !== 'scanner' ? 'hidden lg:block' : ''} h-full p-3 lg:p-4`}>
                            <LiveExecution />
                         </div>
                      )}
                ...
                <div className="hidden lg:block h-full shrink-0 bg-slate-950 border-l border-white/5 overflow-y-auto">
                   <TradeHistory />
                </div>
            </div>
