...
import { Settings2, Percent, DollarSign, RotateCcw } from 'lucide-react';
...
interface AdminStrategyManagerProps {
  ...
  onResetStrategyStats?: (id: string) => void;
  onResetAllStrategies?: () => void;
}

const AdminStrategyManager: React.FC<AdminStrategyManagerProps> = ({ settings, toggleStrategy, saveConfig, onUpdateStrategyConfig, onResetStrategyStats, onResetAllStrategies }) => {
  ...
  const renderStrategyRow = (strat: StrategyDefinition) => {
    ...
    return (
      <tr key={strat.id} className={`border-b border-white/5 transition-colors ${isEnabled ? 'bg-gold-500/5 hover:bg-gold-500/10' : 'bg-slate-950 opacity-40'}`}>
            ...
            <td className="p-4 min-w-[180px]">
                <div className="flex items-center gap-2 bg-slate-950 border border-white/5 p-1 rounded-xl">
                    ...
                    <input 
                        ...
                        className="bg-transparent text-white font-mono text-xs w-20 outline-none text-right pr-2"
                    />
                    <button 
                        onClick={() => {
                            if (onUpdateStrategyConfig) {
                                onUpdateStrategyConfig(strat.id, {
                                    entryType: settings.entryType,
                                    entryValue: settings.entryType === 'FIXED' ? settings.entryFixedValue : settings.entryPercent
                                });
                            }
                        ...
                        className="p-2 rounded-lg text-slate-600 hover:text-gold-500 hover:bg-white/5 transition-colors"
                    >
        ...
        <td className="p-4 text-right font-mono font-bold text-sm text-white">
          <div className="flex items-center justify-end gap-3">
            <span>${stats.profit.toFixed(2)}</span>
            <button 
              onClick={() => {
                if (window.confirm(`Deseja resetar as estatísticas da estratégia ${strat.name}?`)) {
                  if (onResetStrategyStats) onResetStrategyStats(strat.id);
                }
              }}
              title="Resetar Lucro/Eficiência"
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-right-4">
      <div className="flex justify-between items-end pb-8 border-b border-white/5">
        ...
        <div className="flex gap-4">
          <button 
            onClick={() => {
              if (window.confirm("Deseja resetar TODAS as configurações e estatísticas de estratégias?")) {
                if (onResetAllStrategies) onResetAllStrategies();
              }
            }} 
            className="px-6 py-4 bg-slate-800 hover:bg-slate-700 text-white font-black uppercase tracking-widest rounded-2xl transition-all border border-white/5"
          >
            RESETAR TUDO
          </button>
